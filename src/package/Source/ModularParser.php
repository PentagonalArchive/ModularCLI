...
use Pentagonal\ModularCLI\Source\Abstracts\ModularAbstract;
use Pentagonal\ModularCLI\Source\Exceptions\EmptyFileException;
use Pentagonal\ModularCLI\Source\Exceptions\InvalidModularException;
use Pentagonal\ModularCLI\Source\Exceptions\InvalidPathException;
    ...
    protected function setFileToLoad(string $file) : ModularParser
    {
        if (file_exists($file)) {
            ...
            if (strtolower($spl->getExtension()) !== 'php') {
                throw new InvalidArgumentException(
                    sprintf(
                        "%s file has invalid extension. ExtensionParser must be as `php`",
                        $this->getName()
                    ),
                    E_WARNING
                );
